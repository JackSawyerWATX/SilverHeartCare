...
function getSsrHandler(pluginList) {
  const ssrPlugin = pluginList.find(({
    name
  }) => name === "ssr");
  return ssrPlugin;
}
...
function ScrollBody(location, offsetLocation, previousLocation, target, baseDuration, baseFriction) {
  ...
  function settled() {
    ...
    return mathAbs(displacement) < 1e-3;
  }
  function duration() {
  ...
  function getSnapsBySlide() {
    ...
    slidesBySnap.forEach((slideGroup, snapIndex) => {
      slideGroup.forEach((slideIndex) => {
        snapBySlide2[slideIndex] = snapIndex;
      });
    });
    return snapBySlide2;
  }
...
function OptionsHandler() {
  ...
  function optionsAtMedia(options) {
    ...
    const matchedMediaOptions = objectKeys(optionsAtMedia2).filter((media) => windowInstance.matchMedia(media).matches).map((media) => optionsAtMedia2[media]).reduce((mediaOptions, mediaOption) => mergeOptions(mediaOptions, mediaOption), {});
...
function EmblaCarousel(userRoot, userOptions, userPlugins) {
  ...
  let container;
  let slides;
  function cloneEngine(userOptions2) {
    const engineOptions = mergeOptions(options, userOptions2);
    return createEngine(engineOptions, container, slides, true);
  }
  function createEngine(options2, container2, slides2, useCachedRects) {
    const rects = nodeHandler.getRects(container2, slides2, useCachedRects);
    const engine2 = Engine(root, container2, slides2, options2, nodeHandler, eventHandler, rects, isSsr);
  ...
  function activate(withOptions, withPlugins) {
    ...
    let nodes = nodeHandler.getNodes(options);
    const ssrHandler = getSsrHandler(pluginList);
    if (isSsr && ssrHandler) {
      ssrHandler.setup(createEngine, mergeOptions, optionsBase);
      engineOptions = mergeOptions(engineOptions, {
        direction: "ltr"
      });
      nodes = ssrHandler.getNodes();
    }
    root = nodes.root;
    container = nodes.container;
    slides = nodes.slides;
    engine = createEngine(engineOptions, container, slides);
...
function useEmblaCarousel(options = {}, plugins = []) {
  ...
  const serverApi = (0, import_react.useRef)(null);
  const [clientApi, setClientApi] = (0, import_react.useState)();
  const [rootNode, setRootNode] = (0, import_react.useState)();
